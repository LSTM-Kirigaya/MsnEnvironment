...
def control_miniBox(robot_id, instruction, physicsClientId : int = 0):
    """
        |index|direction|
        |:-:|:-:|
        |0|forward|
        ...
        |7|right+backward|
    """
def control_miniBox(robot_id, instruction, target_velocity=TARGET_VELOCITY, multiply=MULTIPLY, 
                    left_wheel_joint_index=LEFT_WHEEL_JOINT_INDEX, right_wheel_joint_index=RIGHT_WHEEL_JOINT_INDEX, 
                    max_force=MAX_FORCE, physicsClientId : int = 0):
    """
        |index|direction|
        |:-:|:-:|
        |0|forward|
...
# 根据输入的键盘字典执行小车运动
def keyboard_control_miniBox(robot_id, key_dict : dict = None, physicsClientId : int = 0):
    if key_dict is None:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # 读取并执行键盘信息
        key_dict = p.getKeyboardEvents()
        keyboard_control_miniBox(robot_id, key_dict)
        # 读取并更新slider中的参数
